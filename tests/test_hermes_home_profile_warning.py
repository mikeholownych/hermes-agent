...
profile is active, get_hermes_home() should:
  1. STILL return ~/.charterforge (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.charterforge."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".charterforge"
        assert "HERMES_HOME fallback" not in capsys.readouterr().err

    def test_default_active_profile_no_warning(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=default → still no warning, returns ~/.charterforge."""
        hermes_dir = tmp_path / ".charterforge"
        hermes_dir.mkdir()
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        assert err.count("CHARTERFORGE_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "CHARTERFORGE_HOME fallback" not in err2
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".charterforge" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".charterforge" / "active_profile").write_text("coder\n")
