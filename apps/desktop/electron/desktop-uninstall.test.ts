...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Charterforge.app/Contents/MacOS/Charterforge', 'darwin'),
    '/Applications/Charterforge.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Charterforge.app/Contents/MacOS/Charterforge', 'darwin'),
    '/Users/x/Applications/Charterforge.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Charterforge\\Charterforge.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Charterforge'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Charterforge.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Charterforge.exe', 'win32'), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Charterforge.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Charterforge.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  ...
  assert.match(script, /export CHARTERFORGE_HOME='\/home\/x\/\.hermes'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Charterforge',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Charterforge" >nul 2>&1/)
