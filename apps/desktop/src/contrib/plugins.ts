 ...
 *    `CharterforgePlugin` registers automatically (vite glob — drop a folder in).
...
const modules = import.meta.glob<{ default: CharterforgePlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default CharterforgePlugin export — skipped`)
