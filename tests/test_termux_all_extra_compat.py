...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"charterforge[termux]"' in text
    assert '"charterforge[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"charterforge[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
