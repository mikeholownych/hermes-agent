...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            ...
            Charterforge profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Charterforge profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    ...
    manager. Starting Charterforge's detached watcher as well would escape the
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Charterforge Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Charterforge Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Charterforge root to compare against. Used when
...
# Legacy service names from older Charterforge installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Charterforge gateway units.

    Detects unit files installed by older Charterforge versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Charterforge gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Charterforge gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Charterforge gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Charterforge gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Charterforge gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Charterforge gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Charterforge often sets ``HOME`` to a profile-scoped directory, but
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Charterforge install"
    )
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Charterforge install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    # Charterforge PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Charterforge PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Charterforge install")
    else:
        print("⚠ Service definition is stale relative to the current Charterforge install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Charterforge gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Charterforge command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Charterforge Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Charterforge delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Charterforge connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Charterforge will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Charterforge will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Charterforge will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy Charterforge gateway unit files from
