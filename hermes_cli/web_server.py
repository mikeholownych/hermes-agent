...
app = FastAPI(title="Charterforge", version=__version__, lifespan=_lifespan)
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Charterforge-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Charterforge (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Charterforge provider slug — fine.
    ...
    1. Vendor-name → Charterforge-provider mapping: when the provider string is
       not a known Charterforge provider/alias (e.g. ``moonshotai``, ``x-ai`` is
...
def _count_status_active_sessions() -> int:
    ...
    while another Charterforge process is writing to it.
    """
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Charterforge credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the Charterforge source tree),
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Charterforge root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Charterforge inside an xterm.js PTY. Browser
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Charterforge Cloud
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Charterforge updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    """Report whether a Charterforge update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed Charterforge version string
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Charterforge surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Charterforge supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Charterforge from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Charterforge to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Charterforge from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Charterforge to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Charterforge in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Charterforge through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Charterforge via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Charterforge through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Charterforge to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Charterforge inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Charterforge to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Charterforge through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Charterforge to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Charterforge to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Charterforge as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Charterforge").strip() or "Charterforge"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Charterforge appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Charterforge-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Charterforge PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Charterforge even
    when they also have a separate Charterforge-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Charterforge never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Charterforge, so the disconnect
    ...
    user sees exactly what executes, and Charterforge then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Charterforge — run the disconnect command to remove it."
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Charterforge-managed PKCE file and auth-store entry.
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Charterforge file AND credential pool.
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Charterforge "
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Charterforge backup archive, while this endpoint is scoped to
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Charterforge home so the job's store,
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Charterforge and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Charterforge for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Charterforge.</p>")
...
_SKILL_HUB_SOURCE_LABELS = {
    ...
    "hermes-index": "Charterforge Index",
    "skills-sh": "skills.sh",
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Charterforge,
...
# ---------------------------------------------------------------------------
# /api/console — safe Charterforge Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Charterforge CLI
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Charterforge Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Charterforge inside WSL2 to use the dashboard's /chat "
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Charterforge Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Without this the SPA paints the default Charterforge Teal canvas, then
    ...
    # (the MC Pages app), not the Charterforge backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Charterforge Teal",         "description": "Classic dark teal — the canonical Charterforge look"},
    {"name": "default-large", "label": "Charterforge Teal (Large)", "description": "Charterforge Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Charterforge exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Charterforge backend listening on {host}:{actual_port}")
            else:
                print(f"  Charterforge Web UI → http://{host}:{actual_port}")
