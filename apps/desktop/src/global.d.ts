import type { GatewayWsUrlResult } from '@charterforge/shared'
...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<CharterforgeConnection>
      ...
      // Charterforge Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CHARTERFORGE_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: CharterforgeApiRequest) => Promise<T>
      notify: (payload: CharterforgeNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<CharterforgeReadFileTextResult>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<CharterforgePreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<CharterforgePreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: CharterforgeTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<CharterforgeReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<CharterforgeGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<CharterforgeGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<CharterforgeGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<CharterforgeRepoStatus | null>
        ...
        review: {
          ...
          shipInfo: (repoPath: string) => Promise<CharterforgeReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: CharterforgeTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<CharterforgeTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: CharterforgeWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: CharterforgePreviewFileChanged) => void) => () => void
...
export interface CharterforgeTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface CharterforgeTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface CharterforgeConnection {
  baseUrl: string
  ...
  remoteCharterforgeVersion?: string
...
export interface CharterforgeTitleBarTheme {
  background: string
  foreground: string
}

export interface CharterforgeWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Charterforge Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Charterforge Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Charterforge Cloud org (slug or id) to
...
// A discovered Charterforge Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface CharterforgeApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface CharterforgeNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface CharterforgePreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface CharterforgeReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface CharterforgePreviewWatch {
  id: string
  path: string
}
...
export interface CharterforgeGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface CharterforgeGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface CharterforgeGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface CharterforgeRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface CharterforgeRepoStatus {
  branch: null | string
  ...
  files: CharterforgeRepoStatusFile[]
}
...
// turn began.
export type CharterforgeReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface CharterforgeReviewFile {
  path: string
  added: number
  removed: number
...
export interface CharterforgeReviewList {
  files: CharterforgeReviewFile[]
...
export interface CharterforgeReviewPr {
  url: string
  state: string
  number: number
}
...
export interface CharterforgeReviewShipInfo {
  ghReady: boolean
  pr: CharterforgeReviewPr | null
}

export interface CharterforgeReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface CharterforgeReadDirResult {
  entries: CharterforgeReadDirEntry[]
  error?: string
}

export interface CharterforgePreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface CharterforgeSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
