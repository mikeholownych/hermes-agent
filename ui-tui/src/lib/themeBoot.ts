...
const bootFilePath = () =>
  join(process.env.CHARTERFORGE_HOME ?? join(homedir(), '.charterforge'), 'tui-theme-boot.json')
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into CHARTERFORGE_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.CHARTERFORGE_TUI_THEME || env.CHARTERFORGE_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.CHARTERFORGE_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.CHARTERFORGE_TUI_BACKGROUND
  ) {
    env.CHARTERFORGE_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.CHARTERFORGE_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.CHARTERFORGE_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into CHARTERFORGE_TUI_THEME.
